...
kitty @ new-window --cwd ~/Layers/layers-core-id zsh -c "source ~/.zshrc && nvm use && yarn start; zsh"

echo "Running core-app-web"
kitty @ new-window --cwd ~/Layers/layers-core-app-web zsh -c "source ~/.zshrc && nvm use && yarn start; zsh"
